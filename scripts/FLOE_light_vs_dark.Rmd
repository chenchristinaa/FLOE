...
compare_sample <- sample %>%
  ...
  filter(population == "STDI" | population == "STGL2" | population == "STIN" | population == "STTO-BH") %>%
  filter(sample != "i9b")
```
...
# to be removed: f26, i07, i08, i10, i26, i27, i29, k07, k26, m07, m30
```

## Removal of Non-normalized

```{r removing the non-normalized}
to_remove <- c("f26", "i07", "i08", "i10", "i26", "i27", "i29", "k07", "k09", "k26", "m07", "m30")
compare_sample <- compare_sample %>%
...
compare_sdiv %>%
  ...
  cor() %>%
  gplots::heatmap.2(Rowv=FALSE, Colv=FALSE, dendrogram="none", trace = "none", col=viridis::viridis(25, begin=.25), margins=c(7,8))
# looks better, but i and k populations are still a mixed
```
...
$Y = \beta_0 + \beta_1X_{STGL2} + \beta_2X_{STIN} + \beta_3X_{STTO-BH} + \beta_4X_{Light} + \beta_5X_{STGL2}X_{Light} + \beta_6X_{STIN}X_{Light} + \beta_7X_{STTO-BH}X_{Light} + \epsilon$

```{r make a model matrix}
design <- model.matrix(~ population * condition, data = compare_sample)
...
$Y = \beta_0 + \beta_1X_{Light} + \beta_2X_{STGL2}X_{Light} + \epsilon$

```{r finding differentially expressed genes again}
STGL2_compare_lrt <- glmLRT(fit, coef = colnames(design[, c(2, 5, 6)]))
```

```{r view the differentially expressed genes again}
topTags(STGL2_compare_lrt)
```

```{r summarise number of differentially expressed genes again}
#summary(decideTestsDGE(STGL2_compare_lrt, p.value=0.01))
summary(decideTests(STGL2_compare_lrt, p.value=0.01))
```

### STIN

```{r finding differentially expressed genes again}
STIN_compare_lrt <- glmLRT(fit, coef = colnames(design[, c(3, 5, 7)]))
```

```{r view the differentially expressed genes again}
topTags(STIN_compare_lrt)
```

```{r summarise number of differentially expressed genes again}
#summary(decideTestsDGE(STIN_compare_lrt, p.value=0.01))
summary(decideTests(STIN_compare_lrt, p.value=0.01))
```

### STTO-BH

```{r finding differentially expressed genes again}
STTO_BH_compare_lrt <- glmLRT(fit, coef = colnames(design[, c(4, 5, 8)]))
```

```{r view the differentially expressed genes again}
topTags(STTO_BH_compare_lrt)
```

```{r summarise number of differentially expressed genes again}
#summary(decideTestsDGE(STTO_BH_compare_lrt, p.value=0.01))
summary(decideTests(STTO_BH_compare_lrt, p.value=0.01))
```
...
```{r finding differentially expressed genes again: conditionLight}
compare_lrt1 <- glmLRT(fit, coef = "conditionLight")
```

```{r view the differentially expressed genes again: conditionLight}
topTags(compare_lrt1)
```

```{r summarise number of differentially expressed genes again: conditionLight}
#summary(decideTestsDGE(compare_lrt, p.value=0.01))
summary(decideTests(compare_lrt1, p.value=0.01))
```

### conditionLight and all light interactions

```{r finding differentially expressed genes again: conditionLight and all light interactions}
compare_lrt2 <- glmLRT(fit, coef = colnames(design[, c(5:8)]))
```

```{r view the differentially expressed genes again: conditionLight and all light interactions}
topTags(compare_lrt2)
```

```{r summarise number of differentially expressed genes again: conditionLight and all light interactions}
#summary(decideTestsDGE(compare_lrt, p.value=0.01))
summary(decideTests(compare_lrt2, p.value=0.01))
```

### all light interactions

```{r finding differentially expressed genes again: all light interactions}
compare_lrt3 <- glmLRT(fit, coef = colnames(design[, c(6:8)]))
```

```{r view the differentially expressed genes again: all light interactions}
topTags(compare_lrt3)
```

```{r summarise number of differentially expressed genes again: all light interactions}
#summary(decideTestsDGE(compare_lrt, p.value=0.01))
summary(decideTests(compare_lrt3, p.value=0.01))
```
